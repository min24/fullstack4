<template>
  <div id="app">
    ...
    <div id="form">
      <div id="ques" v-for="(ques, index) in questions" :key="index"
      v-show="index == indexDisplayed">
        <span>{{ ques.quiz.text }}</span>

        <div id="dapAn"
        v-for="(an, index) in ques.ans" 
        :key="index"
        >
          <input 
          :type="ques.inpt" 
          ...
          v-model="an.selected"
          >
          <span :class="[{red: an.selected == true & an.correct == false},
      ...
      <div id="footer">
        <span @click="indexDisplayed = (indexDisplayed + questions.length - 1) % (questions.length)">Previous</span>
